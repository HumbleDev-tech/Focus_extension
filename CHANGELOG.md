...
- **Eradication of Synthetic Resize Event Storms:** Eliminated global `window.dispatchEvent(new Event('resize'))` in `schedulePlayerResize` and `scheduleChatResize`. Resizing now calls YouTube Polymer's internal `ytd-watch-flexy.handleResize_()` directly, avoiding expensive browser-wide reflows and layout recalculations across the entire DOM tree.
...
- **Decoupled DOM Cleanup Execution:** Removed duplicate, redundant invocations of `cleanLiveChat`, `cleanExplore`, and `cleanSidebar` from `applyStyles`. Mutation cleanup now runs independently and throttled on navigation and relevant DOM mutations only.
